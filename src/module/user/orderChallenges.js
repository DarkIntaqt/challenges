...
export function removeUnnecessaryChallenges(
  challengesArray,
  filters,
  masterOnly = false,
  pointsOnly = false,
  ...
  const challengesDetailed = (window.JSONPREQUEST || { challenges: [] })
    .challenges;

  return challengesDetailed
    .map(function (challengeData) {
      ...
      if (challengeData.queueIds.length > 0) {
        for (let i = 0; i < challengeData.queueIds.length; i++) {
          ...
          if ([450, 930, 860].includes(queue)) {
            pushLater = "aram";
            if (
              filters.gamemode.length > 0 &&
              !filters.gamemode.includes("aram")
            ) {
              return null;
            }
          }
          ...
          if ([400, 420, 430, 440].includes(queue)) {
            ...
            if (
              filters.gamemode.length > 0 &&
              !filters.gamemode.includes("summonersrift")
            ) {
              return null;
            }
          }
          ...
          if ([1700, 1701, 1704].includes(queue)) {
            ...
            if (
              filters.gamemode.length > 0 &&
              !filters.gamemode.includes("arena")
            ) {
              return null;
            }
          }
        ...
        if (
          filters.gamemode.includes("aram") &&
          [101000, 101300, 101200, 101100].includes(challengeData.id)
        ) {
          pushLater = "aram";
        } else {
          return null;
      ...
      if (checkExists(challengeData.tags["parent"])) {
        ...
        [600006, 600010, 600011, 600012, 0].includes(challengeData.id)
      ) {
        parentId = 600006;
      ...
      if (
        ...
        !filters.category.includes(parentId.toString())
      ) {
      ...
      for (let i = 0; i < searchSplit.length; i++) {
        ...
        if (
          ...
          (challenge[6].translation.name
            ...
            challenge[6].translation.description
              .toLowerCase()
              .search(key.toLowerCase()) > -1)
        ) {
          found = true;
...
export default function orderChallenges(
  challenges,
  filter,
  extraFilter,
  masterOnly,
  ...
  switch (filter) {
    ...
    case "levelup":
      sortAlgorithm = function (a, b) {
        ...
        if (
          checkExists(challenge["thresholds"][getNextLevel(tierA, masterOnly)])
        ) {
          nextLevelA = challenge["thresholds"][getNextLevel(tierA, masterOnly)];
        } else {
          nextLevelA = challenge["thresholds"][tierA]
            ? challenge["thresholds"][tierA]
            : 1;
        }
        ...
        if (
          checkExists(challenge["thresholds"][getNextLevel(tierB, masterOnly)])
        ) {
          nextLevelB = challenge["thresholds"][getNextLevel(tierB, masterOnly)];
        } else {
          nextLevelB = challenge["thresholds"][tierB]
            ? challenge["thresholds"][tierB]
            : 1;
        }
        if (challenge.tier === "CHALLENGER" && challenge.leaderboard === true) {
        ...
        return Math.round(nextLevelA * 50) > Math.round(nextLevelB * 50)
          ? -1
          : 1;
      };
      break;
    ...
    case "alphabetic-a-z":
      sortAlgorithm = function (a, b) {
        ...
        return challenge[0]["translation"]["name"] <
          challenge[1]["translation"]["name"]
          ? -1
          : +(
              challenge[0]["translation"]["name"] >
              challenge[1]["translation"]["name"]
            );
      };
      break;
