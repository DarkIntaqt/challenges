...
class Challenges extends Component {
  constructor(props) {
    ...
    this.filter = getStorage(storageKeys.challengeFilters, {
      category: [],
      type: [],
      gamemode: [],
    });
  ...
  load() {
    ...
    get(
      `https://challenges.darkintaqt.com/api/dynamic-data/serve?region=euw1&lang=${window.language}`,
      this.loadChallenges
    );
  }
  ...
  showChallenges(challengeData) {
    ...
    challenges.sort(function (a, b) {
      return a["translation"]["name"] < b["translation"]["name"]
        ? -1
        : +(a["translation"]["name"] > b["translation"]["name"]);
    });
    ...
    for (let i = 0; i < challenges.length; i++) {
      ...
      try {
        if (checkExists(challenge.tags["parent"])) {
          ...
          while (c < 10) {
            ...
            if (
              checkExists(currentChallenge["tags"]["parent"]) &&
        ...
        if (challenge.tags["source"] === "CHALLENGES") {
          if (
            ...
            !this.filter.type.includes("progress")
          ) {
            continue;
          }
          obtainable.push(
            <div key={challenge.tags["source"] + i}>
        ...
        if (challenge.tags["source"] === "EOGD") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("ingame")
          ) {
            continue;
          }
        ...
        if (challenge.tags["source"] === "CAP_INVENTORY") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("inventory")
          ) {
            continue;
          }
          obtainable.push(
        ...
        if (challenge.tags["source"] === "CLASH") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("clash")
          ) {
            continue;
          }
          obtainable.push(
        ...
        if (challenge.tags["source"] === "SUMMONER") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("profile")
          ) {
            continue;
          }
          obtainable.push(
        ...
        if (challenge.tags["source"] === "ETERNALS") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("eternals")
          ) {
            continue;
          }
          obtainable.push(
        ...
        if (challenge.tags["source"] === "RANKED") {
          if (
            this.filter.type.length > 0 &&
            !this.filter.type.includes("ranked")
          ) {
            continue;
          }
          // obtainable.push(<div key={challenge.tags["source"] + i}>
        ...
        if (enabled["isAram"] && enabled["isSR"] && enabled["isArena"]) {
          queueIds.push(
            <div key={"all " + i}>
              ...
              <img
                key={0}
                src="https://cdn.darkintaqt.com/lol/static/lanes/FILL.png"
                alt="All modes"
              />
            </div>
          );
          ...
          if (
            this.filter.gamemode.length > 0 &&
            !this.filter.gamemode.includes("aram")
          ) {
            continue;
          }
          queueIds.push(
          ...
          if (
            this.filter.gamemode.length > 0 &&
            !this.filter.gamemode.includes("summonersrift")
          ) {
            continue;
          }
          queueIds.push(
            <div key={"sr " + i}>
              ...
              <img
                key={2}
                src={config.images.summonersrift}
                alt="Summoners Rift games only"
              />
            </div>
          );
        } else if (
          ...
          if (
            this.filter.gamemode.length > 0 &&
            !this.filter.gamemode.includes("arena")
          ) {
            continue;
          }
          queueIds.push(
          ...
          if (
            this.filter.gamemode.length > 0 &&
            !this.filter.gamemode.includes("bot")
          ) {
            continue;
          }
          queueIds.push(
      ...
      if (
        ...
        !this.filter.category.includes(parentId.toString())
      ) {
              ...
              <div>
                <p>
                  {parentName.charAt(0).toUpperCase() +
                    parentName.slice(1).toLowerCase()}{" "}
                  Category
                </p>
                {config.images[parentId] ? (
                  <img
                    src={config.images[parentId]}
                    alt={parentName.toLowerCase()}
                  />
                ) : null}
              </div>
              {queueIds}
  ...
  applyFilterStyles() {
    ...
    for (const button of buttons) {
      ...
      if (dataFilters.includes(dataId))
      ...
      <Wrapper>
          ...
          <section className={css.parent} style={{ position: "relative" }}>
            <button
              ...
              onClick={() => {
                ...
                setStorage(storageKeys.event, false);
              }}
            >
          ...
          <div className={filterCSS.selectors + " clearfix"}>
            <div className={filterCSS.displayMode}>
              <button
                ...
                className={
                  filterCSS["cmode" + window.compactMode] +
                  " " +
                  filterCSS.modetrue
                }
              >
        ...
        <div className={css.parent + " " + css.flexWidth}>
          ...
          {this.state.showEvent ? null : (
            <button
              className={challengeCSS.showAgain}
              onClick={() => {
                this.setState({ showEvent: true });
                setStorage(storageKeys.event, true);
              }}
            >
              Show banner again
            </button>
