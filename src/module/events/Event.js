...
export default class generateChallengePointElement extends Component {
  constructor(props) {
    ...
    this.state = {
      content: <div className={cCss.crystal + " NONE"}></div>,
    };
  }
  ...
  updateCompontent(leaderboards) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      topPlayer.push(
        <div key={"s" + i} onClick={goTo} className={css.player}>
          <p className={css.position}>{i + 1}.</p>
          <LazyLoadImage
            height={26}
            width={26}
            placeholderSrc={"https://lolcdn.darkintaqt.com/cdn/profileicon/29"}
            src={"https://lolcdn.darkintaqt.com/cdn/profileicon/" + player[3]}
            alt={""}
          ></LazyLoadImage>
          <p className={css.name}>{player[0]}</p>
    ...
    this.setState({
      content: (
        <a
          href="/challenge/2024200"
          className={css.crystal + " CHALLENGER " + cCss.crystal}
          onClick={goTo}
          key="banner"
        >
          ...
          <div className={css.miniLeaderboard}>{topPlayer}</div>
        </a>
      ),
    });
  }
